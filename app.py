...
def load_data(url, is_kpi=False):
    try:
        df = pd.read_csv(url)
        # Standardize headers and remove invisible characters
        df.columns = df.columns.str.strip().str.replace('\ufeff', '')
        
        # Clean the Email column specifically to prevent filtering issues
        if 'Email' in df.columns:
            df['Email'] = df['Email'].astype(str).str.strip().str.lower()
            
        if is_kpi:
            # Clean numeric columns to handle things like "88.2%"
...
with col2:
    ...
    st.subheader(f"Welcome, {advisor_name}")
...
if kpi_df is not None:
    # Filter KPI data for the specific advisor
    ...
    if freq == "Daily":
        ...
        user_kpi['Week_Range'] = (user_kpi['W_Start'].dt.strftime('%d %b %Y') + " - " + 
                                 (user_kpi['W_Start'] + pd.to_timedelta(6, unit='d')).dt.strftime('%d %b %Y'))
    ...
    if dsat_df is not None:
        ...
        # Filter DSAT by current selection
